        ...
        if type(interfaceJson['extends']) is list:
            ...
            for i in range(len(extendsList)):
                if i is extendsCount - 1:
                    extendsString += '[%s](%s)' % (extendsList[i], createMarkdownLink(extendsList[i]))
                else:
                    extendsString += '[%s](%s), ' % (extendsList[i], createMarkdownLink(extendsList[i]))
            ...
            writeToFile('See also: [%s](%s)' % (interfaceJson['extends'], createMarkdownLink(interfaceJson['extends'])))
...
if __name__ == '__main__':
    ...
    outputFileName = os.path.abspath(args.output)
