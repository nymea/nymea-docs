.. _doc-developers-integrations-plugin-code:

The plugin code
===============


Once a :doc:`plugin JSON <plugin-json>` file is created, the according logic is to be implemented in the plugin code. The plugin code can be created in different programming languages. Currently supported are C++/Qt, Python and JavaScript.

This section assumes that you already have the basic plugin file structure in place either by having created a new plugin using the instructions in :doc:`creating a new plugin <creating-a-new-plugin>` or by cloning and editing an existing plugin.
...
.. raw:: html
   ...
   <div class="nymea-tabs">
     <div class="nymea-tab-list" role="tablist" aria-label="Environment language">
       <button class="nymea-tab" id="env-cpp-tab" type="button" role="tab" aria-selected="true" aria-controls="env-cpp-panel">C++/Qt</button>
       <button class="nymea-tab" id="env-python-tab" type="button" role="tab" aria-selected="false" aria-controls="env-python-panel">Python</button>
       <button class="nymea-tab" id="env-js-tab" type="button" role="tab" aria-selected="false" aria-controls="env-js-panel">JavaScript</button>
     </div>
     <div class="nymea-tab-panel" id="env-cpp-panel" role="tabpanel" aria-labelledby="env-cpp-tab">
       <p>C++/Qt plugins are loaded by nymea using <a href="https://doc.qt.io/qt-6/qpluginloader.html">QPluginLoader</a>. This means, they will be executed within the nymea process and will be able to make it crash. As nymea is a long running process, care must be taken to not leak memory or crash by accessing invalid memory.</p>
       <p>Plugins are not threaded by default. This means a plugin developer must not use blocking code. There is, however, the Qt event loop, all Qt API and the nymea hardware manager API available which caters for writing event loop based asynchronous code. If for any reason blocking code must be used, a plugin developer must spawn and manage their own threads.</p>
       <p>A plugin may link to any C/C++/Qt library but please note that for inclusion of a plugin into the nymea plugin repository, all dependencies must be available in all the supported Ubuntu and Debian versions. In some exceptions it is possible to host dependencies in the nymea package repository too.</p>
     </div>
     <div class="nymea-tab-panel" id="env-python-panel" role="tabpanel" aria-labelledby="env-python-tab" hidden>
       <p>Each Python plugin is ran in a sub-interpreter allocated explicitly for the plugin. The Python version depends on the target distribution a nymea instance is build for, it is greater 3.5 in any case though.</p>
       <p>In addition to that, every plugin gets its own thread pool allocated with a maximum thread count matching the amount of managed things + 2. Every function call into a plugin runs in a separate thread, however, if the thread pool is exhausted, calls will be queued and may time out. This means that it is possible to run blocking code in python (e.g. fetch a url using urllib) but this still must be used with caution as it might exceed the thread pool when multiple long running blocking calls are made.</p>
       <p>Python plugins may use asyncio. The recommended way is to call <code>loop.run_forever()</code> in the plugins <code>init()</code> function and call <code>loop.stop()</code> in the plugins <code>deinit()</code> function. In this case, the asyncio event loop will use one of the plugins allocated threads. Other calls into the plugin should then just use <code>asyncio.run_coroutine_threadsafe(coro, loop)</code> to add their tasks to the event loop.</p>
       <p>Additional threads may be started by a Python plugin using the <code>threading</code> module but note that the plugin is responsible for managing that thread and bringing it down on system shutdown.</p>
       <p>A plugin may import any additional Python modules. For inclusion of a plugin into the nymea plugin repository, all dependencies must be satisfyable by <code>pip3</code> or need to be shipped with the plugin code.</p>
       <p><strong>Note:</strong> Python modules installed system wide (e.g. in <code>dist-packages</code> or <code>site-packages</code>) are <em>NOT</em> loaded.</p>
       <p>The <code>modules</code> subdir in the plugins code directory will be available as import path. <code>pip3</code> may be used in the plugin source dir to install additional modules locally during development:</p>
       <div class="nymea-code-block"><pre><code>$ pip3 install somemodule -t modules</code></pre></div>
       <p>For inclusion in the nymea-plugins repository, specify any additional modules in a <code>requirements.txt</code> file. Note that nymea will require hashes and fixed package versions. In order to easily add a module including version and hashes, the tool <code>hashin</code> may be used:</p>
       <div class="nymea-code-block"><pre><code># Install hashin
   ...
   $ hashin somemodule</code></pre></div>
       <p>Nymea will then take care of installing required dependencies.</p>
     </div>
     <div class="nymea-tab-panel" id="env-js-panel" role="tabpanel" aria-labelledby="env-js-tab" hidden>
       <p>Each JavaScript plugin is ran in a separate JS engine. The JS engine is ECMA-5 compliant but does not support node.js nor has a browsers <code>window</code> object.</p>
       <p>Additional <code>.mjs</code> modules may be imported by installing them in the plugins source code and shipping them with the plugin.</p>
     </div>
   </div>
...
The main entry point for the plugin code is typically located in a file with the same name as the :doc:`plugin JSON <plugin-json>` file but using the typical filename extension for the chosen programming language. For a C++/Qt plugin this is ``.h``/``.cpp``, for a Python plugin ``.py`` and JavaScript plugin it is ``.js``.

.. note::

   While for C++/Qt plugins the file name is only recommended, for Python and JavaScript plugins it is a requirement for the file to be named like the json file.
...
.. raw:: html
   ...
   <div class="nymea-tabs">
     <div class="nymea-tab-list" role="tablist" aria-label="Introduction language">
       <button class="nymea-tab" id="intro-cpp-tab" type="button" role="tab" aria-selected="true" aria-controls="intro-cpp-panel">C++/Qt</button>
       <button class="nymea-tab" id="intro-python-tab" type="button" role="tab" aria-selected="false" aria-controls="intro-python-panel">Python</button>
       <button class="nymea-tab" id="intro-js-tab" type="button" role="tab" aria-selected="false" aria-controls="intro-js-panel">JavaScript</button>
     </div>
     <div class="nymea-tab-panel" id="intro-cpp-panel" role="tabpanel" aria-labelledby="intro-cpp-tab">
       <div class="nymea-code-block"><pre><code>// integrationpluginexample.h
   ...
   void IntegrationPluginExample::thingRemoved(Thing *thing) {
     ...
     <div class="nymea-tab-panel" id="intro-python-panel" role="tabpanel" aria-labelledby="intro-python-tab" hidden>
       <div class="nymea-code-block"><pre><code># integrationpluginexample.py
   ...
   def thingRemoved(thing):
       ...
       # Clean up all data related to this thing</code></pre></div>
     </div>
     <div class="nymea-tab-panel" id="intro-js-panel" role="tabpanel" aria-labelledby="intro-js-tab" hidden>
       <div class="nymea-code-block"><pre><code>// integrationpluginexample.js
...
As described in the :doc:`getting started <getting-started-integration>` section, every entity in a plugin is referenced by an ID and a name. A plugin can use the IDs as defined in the JSON file to identify those entities, however, this is discouraged for the sake of readability. Instead, nymea will provide definitions to those IDs in a more readable manner to the developer.
...
.. list-table::
   ...
   * - ThingClass ID
     - *name* + ``ThingClassId``
     - ``robotThingClassId``
   * - ThingClass paramType ID
     - *thingClass* + ``Thing`` + *name* + ``ParamTypeId``
     - ``robotThingNameParamTypeId``
   * - SettingsType ID
     - *thingClass* + ``Settings`` + *name* + ``ParamTypeId``
     - ``robotSettingsSpeedParamTypeId``
   * - Discovery ParamType ID
     - *thingClass* + ``Discovery`` + *name* + ``ParamTypeId``
     - ``robotDiscoveryLanguageParamTypeId``
   * - EventType ID
     - *thingClass* + *name* + ``EventTypeId``
     - ``robotBlinkedEventTypeId``
   * - Event ParamType ID
     - *thingClass* + *event* + ``Event`` + *name* + ``ParamTypeId``
     - ``robotBlinkedEventColorParamTypeId``
   * - StateType ID
     - *thingClass* + *name* + ``StateTypeId``
     - ``robotConnectedStateTypeId``
   * - ActionType ID
     - *thingClass* + *name* + ``ActionTypeId``
     - ``robotSleepActionTypeId``
   * - Action ParamType ID
     - *thingClass* + *action* + ``Action`` + *name* + ``ParamTypeId``
     - ``robotSleepActionDurationParamTypeId``
...
.. raw:: html
   ...
   <div class="nymea-tabs">
     <div class="nymea-tab-list" role="tablist" aria-label="IDs and names language">
       <button class="nymea-tab" id="ids-json-tab" type="button" role="tab" aria-selected="true" aria-controls="ids-json-panel">Plugin JSON</button>
       <button class="nymea-tab" id="ids-cpp-tab" type="button" role="tab" aria-selected="false" aria-controls="ids-cpp-panel">C++/Qt</button>
       <button class="nymea-tab" id="ids-python-tab" type="button" role="tab" aria-selected="false" aria-controls="ids-python-panel">Python</button>
       <button class="nymea-tab" id="ids-js-tab" type="button" role="tab" aria-selected="false" aria-controls="ids-js-panel">JavaScript</button>
     </div>
     <div class="nymea-tab-panel" id="ids-json-panel" role="tabpanel" aria-labelledby="ids-json-tab">
       <div class="nymea-code-block"><pre><code>// JSON definition
   {
       "vendors": [
           {
     ...
     <div class="nymea-tab-panel" id="ids-cpp-panel" role="tabpanel" aria-labelledby="ids-cpp-tab" hidden>
       <div class="nymea-code-block"><pre><code>// In plugininfo.h
   ...
   ParamTypeId robotSleepActionDurationParamTypeId = ParamTypeId("0170952a-260a-44be-972c-cc34f8bc8f67");</code></pre></div>
     </div>
     <div class="nymea-tab-panel" id="ids-python-panel" role="tabpanel" aria-labelledby="ids-python-tab" hidden>
       <div class="nymea-code-block"><pre><code># Exported by the interpreter into the plugin module
   ...
   robotSleepActionDurationParamTypeId = "0170952a-260a-44be-972c-cc34f8bc8f67";</code></pre></div>
     </div>
     <div class="nymea-tab-panel" id="ids-js-panel" role="tabpanel" aria-labelledby="ids-js-tab" hidden>
       <div class="nymea-code-block"><pre><code>// Exported in the engines global object
   ...
   var robotSleepActionDurationParamTypeId = "0170952a-260a-44be-972c-cc34f8bc8f67";</code></pre></div>
     </div>
   </div>
...
.. raw:: html
   ...
   <div class="nymea-tabs">
     <div class="nymea-tab-list" role="tablist" aria-label="Setup language">
       <button class="nymea-tab" id="setup-cpp-tab" type="button" role="tab" aria-selected="true" aria-controls="setup-cpp-panel">C++/Qt</button>
       <button class="nymea-tab" id="setup-python-tab" type="button" role="tab" aria-selected="false" aria-controls="setup-python-panel">Python</button>
       <button class="nymea-tab" id="setup-js-tab" type="button" role="tab" aria-selected="false" aria-controls="setup-js-panel">JavaScript</button>
     </div>
     <div class="nymea-tab-panel" id="setup-cpp-panel" role="tabpanel" aria-labelledby="setup-cpp-tab">
       <div class="nymea-code-block"><pre><code>void  IntegrationPluginExample::setupThing(ThingSetupInfo *info) {
     ...
     <div class="nymea-tab-panel" id="setup-python-panel" role="tabpanel" aria-labelledby="setup-python-tab" hidden>
       <div class="nymea-code-block"><pre><code>from urllib.request import urlopen, Request
   ...
   def setupThing(info):
       ...
       if response.status == "OK":
           ...
           info.finish(nymea.ThingErrorHarwareFailure, "Error connecting to the device in the network.");</code></pre></div>
     </div>
     <div class="nymea-tab-panel" id="setup-js-panel" role="tabpanel" aria-labelledby="setup-js-tab" hidden>
       <div class="nymea-code-block"><pre><code>export function setupThing(info) {
       // Obtain the devices IP as given by the params
...
In this example a HTTP GET call to a REST API on a device would be made. The IP address is obtained from the thing parameters (NOTE: those must be defined in the plugin's JSON file). Once the network request returns, a lambda function is executed which reads the responses payload of the GET call and if everything is as expected it calls ``finish()`` on the setup info object providing the ``ThingErrorNoError`` status code. If something has gone wrong, it'll call ``finish()`` but with a different error code, in this case ``ThingErrorHarwareFailure`` to indicate the failure to the system. If the setup times out before the GET call returns or the user cancels the setup, the ``info`` object will emit its ``aborted`` signal and destroyed.

.. note::

   The info object must not be accessed after it is finished or aborted. Depending on the language this will result in crashes or undefined behavior.
...
.. raw:: html
   ...
   <div class="nymea-tabs">
     <div class="nymea-tab-list" role="tablist" aria-label="Settings language">
       <button class="nymea-tab" id="settings-cpp-tab" type="button" role="tab" aria-selected="true" aria-controls="settings-cpp-panel">C++/Qt</button>
       <button class="nymea-tab" id="settings-python-tab" type="button" role="tab" aria-selected="false" aria-controls="settings-python-panel">Python</button>
       <button class="nymea-tab" id="settings-js-tab" type="button" role="tab" aria-selected="false" aria-controls="settings-js-panel">JavaScript</button>
     </div>
     <div class="nymea-tab-panel" id="settings-cpp-panel" role="tabpanel" aria-labelledby="settings-cpp-tab">
       <div class="nymea-code-block"><pre><code>void IntegrationPluginExample::setupThing(ThingSetupInfo *info) {
       // Do thing setup
       ...
     ...
     <div class="nymea-tab-panel" id="settings-python-panel" role="tabpanel" aria-labelledby="settings-python-tab" hidden>
       <div class="nymea-code-block"><pre><code>def setupThing(info):
       # Do thing setup
       ...
   ...
   def thingSettingChanged(thing, paramTypeId, value):
       if  paramTypeId == myCoolDeviceSettingRefreshIntervalParamTypeId:
           ...
           # Reschedule polling timer here</code></pre></div>
     </div>
     <div class="nymea-tab-panel" id="settings-js-panel" role="tabpanel" aria-labelledby="settings-js-tab" hidden>
       <div class="nymea-code-block"><pre><code>export function setupThing(info) {
       // Do thing setup
       ...
...
.. raw:: html
   ...
   <div class="nymea-tabs">
     <div class="nymea-tab-list" role="tablist" aria-label="Actions language">
       <button class="nymea-tab" id="actions-cpp-tab" type="button" role="tab" aria-selected="true" aria-controls="actions-cpp-panel">C++/Qt</button>
       <button class="nymea-tab" id="actions-python-tab" type="button" role="tab" aria-selected="false" aria-controls="actions-python-panel">Python</button>
       <button class="nymea-tab" id="actions-js-tab" type="button" role="tab" aria-selected="false" aria-controls="actions-js-panel">JavaScript</button>
     </div>
     <div class="nymea-tab-panel" id="actions-cpp-panel" role="tabpanel" aria-labelledby="actions-cpp-tab">
       <div class="nymea-code-block"><pre><code>void IntegrationPluginExample::executeAction(ThingActionInfo *info) {
       // Obtain the devices IP as given by the params
       QString deviceIp = info-&gt;thing()-&gt;paramValue(exampleThingIpParamTypeId).toString();
     ...
     <div class="nymea-tab-panel" id="actions-python-panel" role="tabpanel" aria-labelledby="actions-python-tab" hidden>
       <div class="nymea-code-block"><pre><code>def executeAction(info):
       # Obtain the devices IP as given by the params
       deviceIp = info.thing.paramValue(exampleThingIpParamTypeId)
       ...
       if response.status == "OK":
           ...
           info.finish(nymea.ThingErrorHarwareFailure, "Error command to the network device.");</code></pre></div>
     </div>
     <div class="nymea-tab-panel" id="actions-js-panel" role="tabpanel" aria-labelledby="actions-js-tab" hidden>
       <div class="nymea-code-block"><pre><code>export function executeAction(info) {
       // Obtain the devices IP as given by the params
       var deviceIp = info.thing.paramValue(exampleThingIpParamTypeId);
...
There's one more thing to note: If this action is an action that changes a things state, the plugin implementation should also update the things state value accordingly. See the `States`_ section below for more information how to manipulate states.
...
.. raw:: html
   ...
   <div class="nymea-tabs">
     <div class="nymea-tab-list" role="tablist" aria-label="Events language">
       <button class="nymea-tab" id="events-cpp-tab" type="button" role="tab" aria-selected="true" aria-controls="events-cpp-panel">C++/Qt</button>
       <button class="nymea-tab" id="events-python-tab" type="button" role="tab" aria-selected="false" aria-controls="events-python-panel">Python</button>
       <button class="nymea-tab" id="events-js-tab" type="button" role="tab" aria-selected="false" aria-controls="events-js-panel">JavaScript</button>
     </div>
     <div class="nymea-tab-panel" id="events-cpp-panel" role="tabpanel" aria-labelledby="events-cpp-tab">
       <div class="nymea-code-block"><pre><code>void IntegrationPluginExample::setupThing(ThingSetupInfo *info) {
   ...
   void IntegrationPlugin::thingRemoved(Thing *thing) {
     ...
     <div class="nymea-tab-panel" id="events-python-panel" role="tabpanel" aria-labelledby="events-python-tab" hidden>
       <div class="nymea-code-block"><pre><code># We'll be using threading.timers to do repeating polling in this example
   import threading
   timer = None
   ...
   def deinit():
       ...
       global timer
       timer.cancel()</code></pre></div>
     </div>
     <div class="nymea-tab-panel" id="events-js-panel" role="tabpanel" aria-labelledby="events-js-tab" hidden>
       <div class="nymea-code-block"><pre><code>var pluginTimer;
...
.. raw:: html
   ...
   <div class="nymea-tabs">
     <div class="nymea-tab-list" role="tablist" aria-label="States language">
       <button class="nymea-tab" id="states-cpp-tab" type="button" role="tab" aria-selected="true" aria-controls="states-cpp-panel">C++/Qt</button>
       <button class="nymea-tab" id="states-python-tab" type="button" role="tab" aria-selected="false" aria-controls="states-python-panel">Python</button>
       <button class="nymea-tab" id="states-js-tab" type="button" role="tab" aria-selected="false" aria-controls="states-js-panel">JavaScript</button>
     </div>
     <div class="nymea-tab-panel" id="states-cpp-panel" role="tabpanel" aria-labelledby="states-cpp-tab">
       <div class="nymea-code-block"><pre><code>void IntegrationPluginExample::setupThing(ThingSetupInfo *info) {
       // Doing the regular setup first...
       ...
     ...
     <div class="nymea-tab-panel" id="states-python-panel" role="tabpanel" aria-labelledby="states-python-tab" hidden>
       <div class="nymea-code-block"><pre><code>def setupThing(info):
       # Doing the regular setup first...
       ...
   ...
   def pollService:
       ...
       # Update the things state value accordingly
       thing.setStateValue(temperatureStateTypeId, temperature):</code></pre></div>
     </div>
     <div class="nymea-tab-panel" id="states-js-panel" role="tabpanel" aria-labelledby="states-js-tab" hidden>
       <div class="nymea-code-block"><pre><code>var pluginTimer;
...
.. raw:: html
   ...
   <div class="nymea-tabs">
     <div class="nymea-tab-list" role="tablist" aria-label="Discovery language">
       <button class="nymea-tab" id="discovery-cpp-tab" type="button" role="tab" aria-selected="true" aria-controls="discovery-cpp-panel">C++/Qt</button>
       <button class="nymea-tab" id="discovery-python-tab" type="button" role="tab" aria-selected="false" aria-controls="discovery-python-panel">Python</button>
       <button class="nymea-tab" id="discovery-js-tab" type="button" role="tab" aria-selected="false" aria-controls="discovery-js-panel">JavaScript</button>
     </div>
     <div class="nymea-tab-panel" id="discovery-cpp-panel" role="tabpanel" aria-labelledby="discovery-cpp-tab">
       <div class="nymea-code-block"><pre><code>void IntegrationPluginExample::discoverThings(ThingDiscoveryInfo *info)
   {
     ...
     <div class="nymea-tab-panel" id="discovery-python-panel" role="tabpanel" aria-labelledby="discovery-python-tab" hidden>
       <div class="nymea-code-block"><pre><code>def discoverThings(info):
       ...
       info.finish(nymea.ThingErrorNoError)</code></pre></div>
     </div>
     <div class="nymea-tab-panel" id="discovery-js-panel" role="tabpanel" aria-labelledby="discovery-js-tab" hidden>
       <div class="nymea-code-block"><pre><code>export function discoverThings(info) {
...
.. raw:: html
   ...
   <div class="nymea-tabs">
     <div class="nymea-tab-list" role="tablist" aria-label="Username/Password startPairing language">
       <button class="nymea-tab" id="pair-user-start-cpp-tab" type="button" role="tab" aria-selected="true" aria-controls="pair-user-start-cpp-panel">C++/Qt</button>
       <button class="nymea-tab" id="pair-user-start-python-tab" type="button" role="tab" aria-selected="false" aria-controls="pair-user-start-python-panel">Python</button>
       <button class="nymea-tab" id="pair-user-start-js-tab" type="button" role="tab" aria-selected="false" aria-controls="pair-user-start-js-panel">JavaScript</button>
     </div>
     <div class="nymea-tab-panel" id="pair-user-start-cpp-panel" role="tabpanel" aria-labelledby="pair-user-start-cpp-tab">
       <div class="nymea-code-block"><pre><code>void IntegrationPluginExample::startPairing(ThingPairingInfo *info)
   {
       info-&gt;finish(Thing::ThingErrorNoError, QT_TR_NOOP("Please enter the login credentials for your device."));
   }</code></pre></div>
     </div>
     <div class="nymea-tab-panel" id="pair-user-start-python-panel" role="tabpanel" aria-labelledby="pair-user-start-python-tab" hidden>
       <div class="nymea-code-block"><pre><code>def startPairing(info):
       info.finish(nymea.ThingErrorNoError, "Please enter the login credentials for your device.");</code></pre></div>
     </div>
     <div class="nymea-tab-panel" id="pair-user-start-js-panel" role="tabpanel" aria-labelledby="pair-user-start-js-tab" hidden>
       <div class="nymea-code-block"><pre><code>export function startPairing(info) {
...
.. raw:: html
   ...
   <div class="nymea-tabs">
     <div class="nymea-tab-list" role="tablist" aria-label="Username/Password confirmPairing language">
       <button class="nymea-tab" id="pair-user-confirm-cpp-tab" type="button" role="tab" aria-selected="true" aria-controls="pair-user-confirm-cpp-panel">C++/Qt</button>
       <button class="nymea-tab" id="pair-user-confirm-python-tab" type="button" role="tab" aria-selected="false" aria-controls="pair-user-confirm-python-panel">Python</button>
       <button class="nymea-tab" id="pair-user-confirm-js-tab" type="button" role="tab" aria-selected="false" aria-controls="pair-user-confirm-js-panel">JavaScript</button>
     </div>
     <div class="nymea-tab-panel" id="pair-user-confirm-cpp-panel" role="tabpanel" aria-labelledby="pair-user-confirm-cpp-tab">
       <div class="nymea-code-block"><pre><code>void IntegrationPluginExample::confirmPairing(ThingPairingInfo *info, const QString &amp;username, const QString &amp;password)
   {
     ...
     <div class="nymea-tab-panel" id="pair-user-confirm-python-panel" role="tabpanel" aria-labelledby="pair-user-confirm-python-tab" hidden>
       <div class="nymea-code-block"><pre><code>def confirmPairing(info, username, secret):
       ...
       if response.data == "SUCCESS":
           ...
           info.finish(nymea.ThingErrorAuthenticationFailure, "Error logging in!")</code></pre></div>
     </div>
     <div class="nymea-tab-panel" id="pair-user-confirm-js-panel" role="tabpanel" aria-labelledby="pair-user-confirm-js-tab" hidden>
       <div class="nymea-code-block"><pre><code>export function confirmPairing(info, username, secret) {
       // Compose a request with the credentials set to test the API for a successful login
...
.. raw:: html
   ...
   <div class="nymea-tabs">
     <div class="nymea-tab-list" role="tablist" aria-label="Push button startPairing language">
       <button class="nymea-tab" id="pair-push-start-cpp-tab" type="button" role="tab" aria-selected="true" aria-controls="pair-push-start-cpp-panel">C++/Qt</button>
       <button class="nymea-tab" id="pair-push-start-python-tab" type="button" role="tab" aria-selected="false" aria-controls="pair-push-start-python-panel">Python</button>
       <button class="nymea-tab" id="pair-push-start-js-tab" type="button" role="tab" aria-selected="false" aria-controls="pair-push-start-js-panel">JavaScript</button>
     </div>
     <div class="nymea-tab-panel" id="pair-push-start-cpp-panel" role="tabpanel" aria-labelledby="pair-push-start-cpp-tab">
       <div class="nymea-code-block"><pre><code>void IntegrationPluginExample::startPairing(ThingPairingInfo *info)
   {
       info-&gt;finish(Thing::ThingErrorNoError, QT_TR_NOOP("Please press the push button on the device and then continue this setup."));
   }</code></pre></div>
     </div>
     <div class="nymea-tab-panel" id="pair-push-start-python-panel" role="tabpanel" aria-labelledby="pair-push-start-python-tab" hidden>
       <div class="nymea-code-block"><pre><code>def startPairing(info):
       info.finish(nymea.ThingErrorNoError, "Please press the push button on the device and then continue this setup.");</code></pre></div>
     </div>
     <div class="nymea-tab-panel" id="pair-push-start-js-panel" role="tabpanel" aria-labelledby="pair-push-start-js-tab" hidden>
       <div class="nymea-code-block"><pre><code>export function startPairing(info) {
...
.. _doc-developers-integrations-plugin-code-browsing:
