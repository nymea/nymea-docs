.. _doc-developers-integrations-building-testing:
...
.. note::
   ...
   This section assumes that your build environment is properly installed. See :doc:`build environment <../build-env>` for more information.
...
Optionally, ``nymea-plugininfocompiler`` can be used to validate the :doc:`JSON file <plugin-json>`. Run ``nymea-plugincompiler`` on the plugins ``.json`` file.
...
The same JSON validation step applies as described for Python plugins above.
...
By default, nymead will only print warning messages, that is, messages that have been produced by ``qCWarning()`` (C++/Qt), ``logger.warn()`` (Python) or ``console.warn()`` (JS). In order to also see debug messages produced by ``qCDebug()`` (C++/Qt), ``logger.log()`` (Python) or ``console.log()`` (JS), each plugin has its own debug category. The debug category for a plugin is defined by the ``name`` property in the plugins :doc:`JSON file <plugin-json>`. In order to see a plugins debug messages, nymead can be started by passing the ``-d <Name>`` option. For example:
...
.. note::
   ...
   This message is printed multiple times.
...
.. note::
   ...
   While Python and JS plugins would not complain about an API mismatch at loading time due to the nature of scripting languages, they may still fail later if the changed API is used. Such plugins should be tested thoroughly on major nymea API changes.
...
When working on a plugin, it is recommended to uninstall the released version using:
...
Ensure the path where the plugin is loaded from, is the one you expect it to be (i.e. pointing to your plugin build directory).
