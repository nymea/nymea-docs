...
automate behaviors, similar to rules, but is aimed at more advanced usage of nymea. While scripting
requires a basic understanding of the JavaScript programming language, it also provides a much more
powerful way to interact with nymea.
...
be found in the main screen's menu under Magic using the script icon in the upper right corner.

.. image:: ../../../../static/img/scripting/scripting1.png
   :alt: nymea script editor
...
nymea scripts follow the QML syntax. Please refer to the `QML Documentation <https://doc.qt.io/qt-6/qtqml-documents-topic.html>`__
for details on the QML language.
A script can be thought of as a small app running inside nymea:core. As soon as a script has been
deployed to nymea:core, it will be loaded and execution starts. The script might then listen to state
changes in the system or use timer events to execute actions in the system.


The nymea:app script editor will report console output for the script as well as errors in the script:

.. image:: ../../../../static/img/scripting/scripting2.png
   :alt: nymea script editor console output
   :class: nymea-doc-image
   :width: 50%
...
At any point while creating a script, the nymea:app script editor can support editing by performing
code completion. Code completion will pop up automatically in many places but can be invoked at any
point by pressing Ctrl+Space, or Cmd+Space on macOS.


.. image:: ../../../../static/img/scripting/scripting3.png
   :alt: nymea script editor code completion
   :class: nymea-doc-image
   :width: 50%
...
The most basic concept in nymea scripting is how to define and refer to certain things. On one hand,
there is the need to reference other parts of the script, like accessing properties and calling
functions of objects. On the other hand, there is the need to reference things, meaning devices and
services in the nymea setup, to react to them or execute actions on them.

id
--


The ``id`` property is part of the QML language. Every element in QML can have an ``id`` property. The
value can be defined freely by the script developer as long as it starts with a lowercase letter and
does not contain special characters. Those IDs give the element a name so that it can be referred to
from other places in the script. It is not mandatory to add such ``id`` properties if it is not required
to wire up elements.
...
The reason for using IDs instead of thing names is that the ID is persistent. The user might rename a
thing and existing scripts will still continue to work. If the thing name was used here, scripts would
be more fragile and break more easily.
...
.. image:: ../../../../static/img/scripting/scripting4.png
   :alt: thingId code completion
   :class: nymea-doc-image
   :width: 50%


Selecting an entry will cause the script editor to fill in the appropriate ID for this thing.


.. image:: ../../../../static/img/scripting/scripting5.png
   :alt: inserted thingId
   :class: nymea-doc-image
   :width: 50%
...
Similar to ``thingId``, the properties ``stateName``, ``eventName`` and ``actionName`` need to be
filled in by choosing possible values from the actual things installed in the nymea system. Depending
on the chosen ``thingId``, the possible values differ. Please use the nymea:app code completion to get
a list to choose from.


.. image:: ../../../../static/img/scripting/scripting6.png
   :alt: stateName, eventName and actionName code completion
   :class: nymea-doc-image
   :width: 50%
...
The script engine supports all the types as found in the `QML Documentation <https://doc.qt.io/qt-6/qtqml-documents-topic.html>`__.
The most useful ones for nymea scripts will likely be:
...
A ``ThingState`` is required to have the ``thingId`` property set in order to identify a thing. In addition to that, setting the ``stateName`` property is required to identify a particular state. For example, a light bulb would have a ``stateName`` called ``"power"``.

Each ``ThingState`` has a ``value`` property which contains the actual current value of the state. For instance, a light bulb will contain ``true`` in the value property when it's turned on and ``false`` when it's off. It can also be turned on or off by setting the ``value`` property accordingly.
...
It requires the ``thingId`` property to be set in order to identify the thing we're watching for events. In addition to that, setting the ``eventName`` property is required to select the actual event. For instance, a wall switch would have a pressed event.
...
A ``ThingAction`` is required to have the ``thingId`` property set in order to identify a particular thing and the ``actionName`` property to match the desired action.
   ...
   ThingAction {
       ...
       onExecuted: {
           ...
           if (triggeredBy == Action.TriggeredByUser) {
               print("The action was executed by the user.") 
           }
       }
   }
   ...
...
An ``InterfaceEvent`` is very similar to ``ThingEvent`` with the difference that it does not only receive events from a particular thing, but instead it can receive events from a whole group of things.
...
An ``InterfaceAction`` is similar to ``ThingAction`` with the difference that it executes actions not only on a single thing, but instead executes the same action on a whole group of things.
...
A ``Thing`` allows interaction with a particular thing, that is, reading and setting state values, executing actions and listening to events. If many events, states or actions are needed from a single thing, this may be more convenient than multiple ``ThingEvent``, ``ThingState`` or ``ThingAction`` items. However, this variant does not allow property bindings as conveniently as the other types. Additionally, this allows retrieving information about a thing, such as its name.
...
``ThingState`` items can be linked using a property binding. For instance, to turn on a light while a presence 
